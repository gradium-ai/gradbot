DEMOS := business_bank chick_fil_a egg_timer fantasy_shop hotel mcp_demo \
         mtg_adviser simple_chat spanish_teacher threejs_game \
         voice_text_adventure web_search wellness_checkin
...
run:
ifndef DEMO
	$(error DEMO is required. Usage: make run DEMO=threejs_game)
endif
	cd demos/$(DEMO) && .venv/bin/uvicorn main:app --reload --port $(PORT) --reload-exclude "$$PWD/.venv"
