...
def get_current_user(
    ...
    user = _resolve_user(db, paris_rental_session)
    if not user:
        raise HTTPException(
...
def get_optional_user(
    ...
    return _resolve_user(db, paris_rental_session)
...
def get_user_from_token_or_cookie(
    ...
    user = _resolve_user(db, paris_rental_session) or _resolve_user(db, token)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Not authenticated",
